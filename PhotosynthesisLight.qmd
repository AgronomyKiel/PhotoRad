---
title: "Analysing leaf CO~2~ assimilation data"
author: "Henning Kage"
date: "`r Sys.Date()`"
...
$$P_{max}=g_s\Delta CO_2$$ This lead to the following equation: $$P_{n}^{{}}=\frac{\alpha \,I\,{{g}_{s}}\Delta C}{\alpha \,I+{{g}_{s}}\,\Delta C}-R_d $$
...
$${{P}^{2}}\Theta -P(\alpha I +{{P}_{\max }})+\alpha I {{P}_{\max }}=0$$ This quadratic equation has two solutions from which only the second with a negative sign before the square root is physiological meaningful:

$${{P}_{1,2}}=\frac{(\alpha I +{{P}_{\max }})\pm \sqrt{{{(\alpha I +{{P}_{\max }})}^{2}}-4\Theta \alpha I {{P}_{\max }}}}{2\Theta }$$
...
$$P_n=P_{max} \left( 1-e^{\frac {\alpha I}{P_{max}}} \right )$$
...
```{r functiondefinition, echo=TRUE}
# Blackman-response curve
...
NegExpoential<- function(Pmax, alpha, I){
...
#rectangular hyperbola with dark respiration term
RectHypR <- function(Pmax, alpha, r, I){
  result <- alpha*Pmax*I/((alpha*I)+Pmax)-r
  return(result)  
}

#non-rectangular hyperbola with dark respiration term and CO2 effect
NonRectHyp <- function(alpha, pmax, theta, par){
  assi <- alpha*par+pmax-sqrt((alpha*par+pmax)^2-4*theta*alpha*par*pmax)/(2*theta)
  return(assi)
}


NonRectHypR <- function(alpha, pmax, theta, r, par){
  assi <- alpha*par+pmax-sqrt((alpha*par+pmax)^2-4*theta*alpha*par*pmax)/(2*theta)-r
  return(assi)
}


#rectangular hyperbola with dark respiration term integrated over the canopy
# assumin Pmax to be constand over the leaf layers of the canopy
RectHypInt <- function(LAI, Pmax, alpha, r, k, I) {
  result <- Pmax/k*log((alpha*k*I+Pmax)/(alpha*k*I*exp(-k*LAI)+Pmax))-r
  return(result)  
}

#rectangular hyperbola with dark respiration term and CO2 effect
RectHyp_gs <- function(alpha, tau, r, par, co2){
  assi <- alpha*par*tau*co2/(alpha*par+tau*co2)-r
  return(assi)
}

#non-rectangular hyperbola with dark respiration term and CO2 effect
NonRectHyp_gs <- function(alpha, tau, theta, r, par, co2){
  assi <- alpha*par+tau*CO2-sqrt((alpha*par+tau*co2)^2-4*theta*alpha*par*tau*co2)/(2*theta)-r
  return(assi)
}
...
mod.negExpo <-  nls(Pbrutto ~ NegExpoential(Pmax, alpha, PAR), data=dat, 
